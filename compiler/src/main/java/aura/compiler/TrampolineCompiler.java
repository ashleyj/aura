...
import aura.compiler.trampoline.*;
...
public class TrampolineCompiler {
    ...
    private FunctionRef createLdcArray(String targetClass) {
        ...
        if (!mb.hasSymbol(fnName)) {
            ...
            Global g = new Global(Symbols.arrayPtrSymbol(targetClass), weak, new NullConstant(Types.OBJECT_PTR));
    ...
    private boolean checkClassAccessible(Function f, Trampoline t) {
        ...
        throwIllegalAccessError(f, Access.ILLEGAL_ACCESS_ERROR_CLASS,
                target, caller);
        f.add(new Unreachable());
        return false;
    }
    ...
    private boolean checkMemberAccessible(Function f, Trampoline t, ClassMember member) {
        ...
        if (member instanceof SootMethod) {
            ...
            throwIllegalAccessError(f, Access.ILLEGAL_ACCESS_ERROR_METHOD,
                    method.getDeclaringClass(), method.getName(), 
                    Types.getDescriptor(method), caller.getSootClass());
        } else {
            SootField field = (SootField) member;
            throwIllegalAccessError(f, Access.ILLEGAL_ACCESS_ERROR_FIELD,
