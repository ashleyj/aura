...
public class MarshalerLookup {
    ...
    public MarshalerMethod findMarshalerMethod(MarshalSite marshalSite) {
        ...
        if (pidx != MarshalSite.RECEIVER) {
            ...
            AnnotationTag anno = pidx == MarshalSite.RETURN_TYPE
                    ? Annotations.getMarshalerAnnotation(marshalSite.method)
                    : Annotations.getMarshalerAnnotation(marshalSite.method, pidx);
            if (anno != null) {
    ...
    private Set<Long> getSupportedCallTypes(AnnotationTag anno) {
        AnnotationArrayElem el = 
                (AnnotationArrayElem) Annotations.getElemByName(anno, "supportedCallTypes");
        if (el == null) {
    ...
    private soot.Type getBaseType(SootMethod m, AnnotationTag anno) {
        ...
        if (t instanceof RefType) {
            ...
            if (arrayType.baseType instanceof PrimType 
                    || Types.isInstanceOfClass(arrayType.baseType, "Struct")) {
    ...
    public static class MarshalSite {
        ...
        public MarshalSite(SootMethod method, int paramIdx) {
            ...
            if (Annotations.hasBridgeAnnotation(method)) {
                ...
                array = paramIdx == RETURN_TYPE ? Annotations.hasArrayAnnotation(method)
                        : Annotations.hasArrayAnnotation(method, paramIdx);
    ...
    public class PointerMarshalerMethod extends MarshalerMethod {
        ...
        public SootMethod getAfterBridgeCallMethod() {
            ...
            for (SootMethod m : method.getDeclaringClass().getMethods()) {
                if (Annotations.hasAfterBridgeCallAnnotation(m)) {
        ...
        public SootMethod getAfterCallbackCallMethod() {
            ...
            for (SootMethod m : method.getDeclaringClass().getMethods()) {
                if (Annotations.hasAfterCallbackCallAnnotation(m)) {
                    if (m.getReturnType() == VoidType.v() 
                            && m.getParameterTypes().equals(paramTypes)) {
    ...
    public class ValueMarshalerMethod extends MarshalerMethod {
        ...
        public Type getNativeType(Arch arch) {
            if (method.getReturnType() instanceof PrimType) {
                ...
                if (arch.is32Bit() && (Annotations.hasMachineSizedSIntAnnotation(method)
                        || Annotations.hasMachineSizedUIntAnnotation(method))) {
                    return Type.I32;
                }
                if (arch.is32Bit() && (Annotations.hasMachineSizedFloatAnnotation(method))) {
                    return Type.FLOAT;
                }
                if (!arch.is32Bit() && (Annotations.hasMachineSizedFloatAnnotation(method))) {
                    return Type.DOUBLE;
                ...
                if (Annotations.hasPointerAnnotation(method, 1)) {
                    return Type.I8_PTR;
                }
                if (arch.is32Bit() && (Annotations.hasMachineSizedSIntAnnotation(method, 1)
                        || Annotations.hasMachineSizedUIntAnnotation(method, 1))) {
                    return Type.I32;
                }
                if (arch.is32Bit() && (Annotations.hasMachineSizedFloatAnnotation(method, 1))) {
                    return Type.FLOAT;
                }
                if (!arch.is32Bit() && (Annotations.hasMachineSizedFloatAnnotation(method, 1))) {
                    return Type.DOUBLE;
            ...
            if (toObjectMethods == null && toNativeMethods == null 
                ...
                for (SootMethod m : clazz.getSootClass().getMethods()) {
                    if (m.isStatic() && m.isPublic()) {
                        ...
                        AnnotationTag valueAnno = Annotations.getMarshalsValueAnnotation(m);
                        AnnotationTag arrayAnno = Annotations.getMarshalsArrayAnnotation(m);
