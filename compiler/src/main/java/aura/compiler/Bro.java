...
public abstract class Bro {
    ...
    public static class MarshalerFlags {
        // The constant here MUST correspond to those in MarshalerFlags in rt.
    ...
    public static int[] getArrayDimensions(SootMethod method, int paramIndex) {
        AnnotationTag annotation = paramIndex == -1 ? Annotations.getArrayAnnotation(method)
                : Annotations.getArrayAnnotation(method, paramIndex);
        if (annotation == null) {
            return null;
        }
    ...
    public static boolean isPassByValue(SootMethod method) {
        ...
        return Types.isStruct(sootType) && (Annotations.hasByValAnnotation(method)
                || Annotations.hasByValAnnotation(((RefType) sootType).getSootClass()));
    }
    ...
    public static boolean isPassByValue(SootMethod method, int paramIndex) {
        ...
        return Types.isStruct(sootType) && (Annotations.hasByValAnnotation(method, paramIndex)
                || Annotations.hasByValAnnotation(((RefType) sootType).getSootClass()));
    }
    ...
    public static boolean isStructRet(SootMethod method, int paramIndex) {
        ...
        return paramIndex == 0 && Types.isStruct(sootType)
                && (Annotations.hasStructRetAnnotation(method, paramIndex));
    }
