...
public class TypeEncoder {
    ...
    private String encodeOne(SootMethod method, Type t, int idx, boolean is64bit) {
        ...
        if (Types.isStruct(method.getDeclaringClass()) && hasAnno(method, idx, Annotations.ARRAY)) {
    ...
    private String encodeRef(SootMethod method, RefType t, int idx, boolean is64bit) {
        ...
        if (Types.isStruct(t)) {
            if (hasAnno(method, idx, Annotations.BY_VAL)) {
                // Encode as struct
    ...
    private String encodePrimitive(SootMethod method, PrimType t, int idx, boolean is64bit) {
        if (t.equals(soot.BooleanType.v())) {
            ...
            if (hasAnno(method, idx, Annotations.POINTER)) {
                return "^v"; // void*
            }
            if (hasAnno(method, idx, Annotations.MACHINE_SIZED_S_INT)) {
                return is64bit ? "q" : "i";
            }
            if (hasAnno(method, idx, Annotations.MACHINE_SIZED_U_INT)) {
                return is64bit ? "Q" : "I";
            }
            return "q";
        } else if (t.equals(soot.FloatType.v())) {
            if (hasAnno(method, idx, Annotations.MACHINE_SIZED_FLOAT)) {
                return is64bit ? "d" : "f";
            }
            return "f";
