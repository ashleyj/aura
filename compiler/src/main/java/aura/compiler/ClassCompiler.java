...
import aura.compiler.util.io.HfsCompressor;
import org.apache.commons.io.FileUtils;
import org.apache.commons.io.IOUtils;
import org.apache.commons.lang3.tuple.Triple;
import aura.compiler.clazz.Clazz;
import aura.compiler.clazz.ClazzInfo;
import aura.compiler.clazz.MethodInfo;
import aura.compiler.llvm.Alias;
import aura.compiler.llvm.AliasRef;
import aura.compiler.llvm.And;
import aura.compiler.llvm.ArrayConstantBuilder;
...
import aura.compiler.llvm.Fence;
import aura.compiler.llvm.Function;
import aura.compiler.llvm.FunctionDeclaration;
...
import aura.compiler.llvm.Linkage;
import aura.compiler.llvm.Load;
import aura.compiler.llvm.NullConstant;
import aura.compiler.llvm.Ordering;
import aura.compiler.llvm.PackedStructureConstantBuilder;
import aura.compiler.llvm.PointerType;
import aura.compiler.llvm.Ret;
import aura.compiler.llvm.Store;
import aura.compiler.llvm.StructureConstant;
...
import aura.compiler.trampoline.Invoke;
import aura.compiler.trampoline.Invokeinterface;
import aura.compiler.trampoline.Invokevirtual;
import aura.llvm.Context;
import aura.llvm.LineInfo;
import aura.llvm.Module;
import aura.llvm.ObjectFile;
import aura.llvm.PassManager;
import aura.llvm.PassManagerBuilder;
import aura.llvm.Symbol;
import aura.llvm.Target;
import aura.llvm.TargetMachine;
import aura.llvm.binding.Attribute;
import aura.llvm.binding.CodeGenFileType;
import aura.llvm.binding.CodeGenOptLevel;
import aura.llvm.binding.RelocMode;
...
public class ClassCompiler {
            ...
            try (Module module = Module.parseIR(context, llData, clazz.getClassName())) {
                ...
                if (!cCode.isEmpty()) {
                    ...
                    try (Module m2 = Module.parseClangString(context, sb.toString(), clazz.getClassName() + ".c", config.getClangTriple())) {
                        module.link(m2);
                        for (aura.llvm.Function f1 : m2.getFunctions()) {
                            String name = f1.getName();
                            aura.llvm.Function f2 = module.getFunctionByName(name);
                            if (Symbols.isBridgeCSymbol(name) || Symbols.isCallbackCSymbol(name) || Symbols.isCallbackInnerCSymbol(name)) {
                                f2.setLinkage(aura.llvm.binding.Linkage.PrivateLinkage);
    ...
    private void compile(Clazz clazz, OutputStream out) throws IOException {
        ...
        for (SootMethod method : sootClass.getMethods()) {
            ...
            if (Annotations.hasBridgeAnnotation(method)) {
                ...
                function = globalValueMethod(method);
                        ...
                        || "sizeOf".equals(name) || Annotations.hasStructMemberAnnotation(method))) {
        ...
        mb.addAlias(new Alias(classInfoStruct.getName() + "_i8ptr", Linkage._private, new ConstantBitcast(
                classInfoStruct.ref(), Type.I8_PTR)));
    ...
    private void createLookupFunction(SootMethod m) {
        ...
        if (!sootClass.isInterface()) {
            ...
            Value classPtr = call(function, Functions.OBJECT_CLASS, function.getParameterRef(1));
            Value vtablePtr = call(function, Functions.CLASS_VITABLE, classPtr);
            ...
            Value fptr = call(function, Functions.BC_LOOKUP_INTERFACE_METHOD_IMPL, args);
            Variable f = function.newVariable(function.getType());
            ...
            for (SootClass ifs : interfaces) {
                ...
                if (itable.size() > 0) {
                    ...
                    if (!mb.hasSymbol(typeInfoName)) {
                        mb.addGlobal(new Global(typeInfoName, Linkage.external, Type.I8_PTR, true));
                    }
            ...
            if (tables.isEmpty()) {
                ...
                Global itablesStruct = new Global(Symbols.itablesSymbol(Types.getInternalName(sootClass)), Linkage._private,
                        new StructureConstantBuilder()
                            ...
                            .add(new ArrayConstantBuilder(Type.I8_PTR).add(tables).build())
                            .build());
    ...
    private StructureConstant createClassInfoStruct() {
        ...
        header.add(new NullConstant(Type.I8_PTR)); // Points to the runtime Class struct
        ...
        mb.addGlobal(new Global(Symbols.typeInfoSymbol(Types.getInternalName(sootClass)), Linkage.external, Type.I8_PTR, true));
        header.add(new GlobalRef(Symbols.typeInfoSymbol(Types.getInternalName(sootClass)), Type.I8_PTR)); // TypeInfo* generated by Linker
        ...
        if (attributesEncoder.classHasAttributes()) {
            body.add(new ConstantBitcast(attributesEncoder.getClassAttributes().ref(), Type.I8_PTR));
        }
        ...
        for (SootField f : sootClass.getFields()) {
            ...
            if (attributesEncoder.fieldHasAttributes(f)) {
                body.add(new ConstantBitcast(attributesEncoder.getFieldAttributes(f).ref(), Type.I8_PTR));
            }
        }
        ...
        for (SootMethod m : sootClass.getMethods()) {
            ...
            if (Modifier.isNative(m.getModifiers())) {
                if (!Types.isStruct(sootClass) && !Annotations.hasStructMemberAnnotation(m)) {
                    flags |= MI_NATIVE;
                }
            }
            ...
            if (attributesEncoder.methodHasAttributes(m)) {
                body.add(new ConstantBitcast(attributesEncoder.getMethodAttributes(m).ref(), Type.I8_PTR));
            }
            if (!m.isAbstract()) {
            ...
            if (Annotations.hasBridgeAnnotation(m)) {
                if (!Annotations.readBooleanElem(Annotations.getAnnotation(m, Annotations.BRIDGE), "dynamic", false)) {
    ...
    private Function createAllocator() {
        ...
        Value result = Functions.call(fn, Functions.BC_ALLOCATE, fn.getParameterRef(0), info);
    ...
    private Function createLdcClass() {
        ...
        Value info = getInfoStruct(fn, sootClass);
        Value result = Functions.call(fn, Functions.BC_LDC_CLASS, fn.getParameterRef(0), info);
        fn.add(new Ret(result));
        return fn;
    }
    ...
    private Function createLdcClassWrapper() {
        ...
        Value info = getInfoStruct(fn, sootClass);
        Value result = Functions.call(fn, Functions.LDC_CLASS_WRAPPER, fn.getParameterRef(0), info);
        fn.add(new Ret(result));
        return fn;
    }
    ...
    private Function createClassInitWrapperFunction(FunctionRef targetFn) {
        ...
        Functions.call(fn, Functions.BC_INITIALIZE_CLASS, fn.getParameterRef(0), info);
    ...
    static Value getInfoStruct(Function f, SootClass sootClass) {
        return Functions.call(f, FunctionBuilder.infoStruct(sootClass).ref());
    }
